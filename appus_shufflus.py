...
questions_file = st.file_uploader(
    "Upload your question CSV (no header needed)",
    type=["csv"]
)

image_file = st.file_uploader(
    "Upload the image to reveal",
    type=["png", "jpg", "jpeg"]
)
...
# Main logic
# -------------------------------------------------
if questions_file and image_file:
    ...
    if tile_width < 10 or tile_height < 10:
        st.error("The image is too small. Please use a larger image or fewer questions.")
        st.stop()

    # -------------------------------------------------
    # Initialize session state (Modified for Shuffling)
    # -------------------------------------------------
    if "iteration" not in st.session_state:
        st.session_state.iteration = 1

    if "shuffled_questions" not in st.session_state:
        # 1. Shuffle the order of the questions themselves
    ...
    # -------------------------------------------------
        ...
        if st.session_state.iteration < 3:
            if st.button(f"Start Attempt {st.session_state.iteration + 1}"):
                # Increment iteration and clear quiz state but keep the iteration count
